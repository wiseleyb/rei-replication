...
class ReplUtils
  class << self
    ...
    def read_slot!
      sql = %(
        SELECT *
        FROM pg_logical_slot_get_changes('#{ReplConfig.slot}',
                                         NULL,
                                         NULL,
                                         'pretty-print',
                                         '1')
    ...
    def peek_slot
      sql = %(
        SELECT *
        FROM pg_logical_slot_peek_changes('#{ReplConfig.slot}',
                                          NULL,
                                          NULL,
                                          'pretty-print',
                                          '1');
    ...
    def create_replication_slot!
      ...
      sql = %(
        SELECT 'init'
        FROM pg_create_logical_replication_slot('#{ReplConfig.slot}',
                                                'wal2json')
      )
      exec_sql(sql)
    end
    ...
    def delete_replication_slot!
      ...
      sql = %(select pg_drop_replication_slot('#{ReplConfig.slot}'))
      exec_sql(sql)
    end
